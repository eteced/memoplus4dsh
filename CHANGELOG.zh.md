...
- **设置卡片分三档，把 11 个键收成"两行 + 一个折叠区"。** 原来 11 个键平铺、没有
  折叠，日常真正会拧的只有 `injectTopK`，其余全是噪音。现在按**"普通用户改它之前要
  不要先知道后果"**分档（不是按重不重要）：`common` 常改始终可见（`injectTopK`；
  以及 `debug` —— 它不是调优而是**一种必须能一眼看到的状态**，收进折叠容易忘了关）；
  `advanced` 重要但少改，折在「高级设置」里，展开后仍逐项带说明与生效语义
  （`promptProfile` / `reasoningEffortPolicy` / `thinkingTokenHeadroom` /
  `extractionMaxFailureRounds`）；其余 5 个（`promptProfilesDir` 与四个队列参数，
  **全部是"重启后生效"**）由折叠区里的一段 JSON 承载，写 `{}` 或清空即全部回到
  默认/继承。折叠标题在**收起状态**就报出项数与"都要重启 dsh 才生效"—— 不用展开
  也知道代价。**每个键只有一个主人**：`raw` 档不再渲染成控件，控件与 JSON 不会
  出现两个写入面。JSON 走同一份 drafts/dirty/save 管线，就地校验：解析失败或出现
  本命名空间以外的键都**阻塞保存并保留草稿**，未知键明确报出来而不是静默丢弃；
  初始文本只收**已覆盖**的键，绝不把 `cordis.yml` 继承值烤成覆盖。
  测试那道"客户端键集不得与 Host 漂移"的门改成**更强的形态**：先静态断言 `SURFACE`
  分档表恰好覆盖 `MEMORY_SETTING_KEYS`（新增 Host 键却忘了分档会直接失败），再断言
  折叠态渲染出的控件恰好是 `common` 那一档。浏览器实测中发现并修掉一个真 bug：
