...
| 维度 | 本组合 | 最佳公开基线 |
|---|---|---|
| **选择性遗忘·单跳**（FC-SH，6k→262k） | **81.25**（83/81/78/83） | 60.0（GPT-4o）；记忆类最高 54.0 |
| **选择性遗忘·多跳**（FC-MH） | **76.0**（75/74/76/79） | **7.0**（全员）；o4-mini 80.0→14.0 断崖 |
| **精确召回**（LME(S*)，LLM judge） | **54.67** | 55.7（GPT-4.1-mini）；记忆类最高 50.7 |

两个关键点：

- **FC-MH 是官方全体 agent ≤7% 的"死亡任务"**，我们 76.0——多跳 + 状态更新恰好是实体图一跳扩展 + 状态去重的设计目标。
- **长度无衰减**：6k 到 262k 两条曲线持平，而所有基线（包括推理模型）都随长度骤降。

另经真人场景测试（goal 跨 session 进度、todo 演进、SIGKILL 崩溃恢复等）全过；121 个单测全绿。

**已知短板**：LME multi-session 32.0（跨 session 时序链整合是检索式记忆的结构性弱项）；ingest 成本高于 embed 类方案（LLM 抽取 vs 向量化）；Zen 类 Go 网关端点有上游 tool_calls bug（known-issues F1，官方 API 无影响）。
