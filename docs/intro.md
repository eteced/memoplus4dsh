...
> 官方数据加载/模板/指标/judge 零修改复用；被测为插件默认配置。细节与口径：[docs/m9-benchmark.md](docs/m9-benchmark.md)。

| 维度 | 本组合 | 最佳公开基线 |
|---|---|---|
| **选择性遗忘·单跳**（FC-SH，6k→262k） | **57.75**（63/52/59/57） | 60.0（GPT-4o 全文塞窗口）；记忆类最高 54.0 → **记忆系统第一** |
| **选择性遗忘·多跳**（FC-MH） | **30.25**（28/38/35/20） | **7.0**（全员）→ **4.3 倍于最佳基线** |
| **精确召回**（LME(S*)，LLM judge） | **56.67** | 55.7（GPT-4.1-mini）→ **全场第一** |

两个关键点：

- **FC-MH 是官方全体 agent ≤7% 的"死亡任务"**，我们 30.25，是唯一在 262k 长上下文多跳遗忘上不失效的记忆系统（o4-mini 32k 即崩至 14.0）。多跳 + 状态更新恰好是实体图一跳扩展 + 状态去重的设计目标。
...
**已知短板**：LME multi-session 42.7（跨 session 时序链整合是检索式记忆的结构性弱项，第二轮数据）；ingest 成本高于 embed 类方案（LLM 抽取 vs 向量化）；Zen 类 Go 网关端点有上游 tool_calls bug（known-issues F1，官方 API 无影响）。
