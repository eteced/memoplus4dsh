...
**已排除的假设（实测，非推断）**：
1. ~~«错并源于裁决阶段关闭 thinking»~~ —— `off` 与 `high` 结果相同，排除；
2. ~~«本脚本的候选集合比线上窄，所以更容易判对»~~ —— `candidatesFor()` 先做子串/token 重叠，**再在存在 embedder 时追加余弦候选**（`src/entity-merge.ts:144`）。加了与真机同向的确定性嵌入桩后，用 `--candidates-only` 逐条对比：候选集合**基本一致**（8 条 mention 里只有 1 条多出 1 个候选），故候选机制不是原因。这条假设是我先猜的，不是证据支持的。
