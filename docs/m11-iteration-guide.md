...
| v3+P1-B | 启发式蒸馏主路 + LLM 逐字引用兜底 + supersede 标记 + 注入近重复抑制 + RANGE 算子 + LLM 实体合并 + 事件同轮去重 | **5/5** | **5/5** | sh 3/5, mh 1/5（5题档） | supersede 精确修复上轮 3 道冲突错题；ingest 提速 3 倍（长消息跳过注入）；**mini-3 又实锤一个 supersede bug**：旧值被重提时反向取代新值（提及顺序≠信息新旧）→ 已修（重提守卫 + 标记传播）；MH 暴露"搜索只到第一跳"→ via 机制（memory_search 附带 top-3 命中实体的最新邻接事实） |
