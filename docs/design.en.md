...
- **Prompt profiles**: one `<name>.prompts` file is one profile, and the file name is the profile name; `model` / `provider` in the header take `*` wildcards (a profile without `model` is reachable only through `promptProfile`), and each `@@ stage <name>`…`@@ end` block carries that stage's prompt **verbatim, with no escaping**. Precedence: `prompts.<stage>` (explicit override) → the selected profile (`promptProfile`, else the first match in declaration order) → the built-in `default`. A profile may cover only some stages; uncovered stages are reported per stage as the built-in default. Five stages: extraction / entityMerge / supersede / queryExpansion / queryDistill.
- **Per call, because the route is per call**: the write path takes the route that turn was recorded with and the query side the session's latest request header; when `extractionProvider` + `extractionModel` are configured both are first replaced by that override. A profile is therefore always matched on the model the call **actually runs on**, never on the one selected in the composer. Switching the model in the Models page takes effect on the **next turn**, with no plugin reload.
...
- **Observability**: `memory_status` reports the configured profiles, the route in hand, **where each stage's prompt actually came from** (the profile, a plugin-config override, or the built-in default) alongside its effective `maxTokens/effort/timeoutMs`, and the live embedding model and dimension; profile selection and switches are written to `extraction-debug.jsonl` (one entry per change).
