...
> Official data loading / templates / metrics / judge reused with zero modifications; the plugin was tested in its default configuration. Details and methodology: [docs/m9-benchmark.md](docs/m9-benchmark.md).

| Dimension | Ours | Best public baseline |
|---|---|---|
| **Selective forgetting · single-hop** (FC-SH, 6k→262k) | **57.75** (63/52/59/57) | 60.0 (GPT-4o full text stuffed into the window); best memory-class system 54.0 → **#1 among memory systems** |
| **Selective forgetting · multi-hop** (FC-MH) | **30.25** (28/38/35/20) | **7.0** (everyone) → **4.3× the best baseline** |
| **Accurate recall** (LME(S*), LLM judge) | **56.67** | 55.7 (GPT-4.1-mini) → **#1 overall** |

Two key points:

- **FC-MH is a "death task" where all official agents score ≤7%**; we scored 30.25, the only memory system that doesn't collapse on 262k long-context multi-hop forgetting (o4-mini collapses to 14.0 at just 32k). Multi-hop + state updates happen to be exactly the design targets of entity-graph one-hop expansion + state dedup.
...
**Known weaknesses**: LME multi-session 42.7 (integrating cross-session temporal chains is a structural weakness of retrieval-based memory; second-round data); ingest cost is higher than embed-based approaches (LLM extraction vs vectorization); Zen-class Go gateway endpoints have an upstream tool_calls bug (known-issues F1, no impact on the official API).
