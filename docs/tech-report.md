...
2. **LLM 裁决的实体合并**（M11，合并由大模型判定而非规则堆叠）：精确名未命中的新提及（主体与客体都算）先做**嵌入相似候选召回**（多语言向量，阈值刻意放宽到 0.6/top-5——粗召回），再一次 LLM 调用逐条裁决"是否同一实体"。**只有显式 `sure` 才合并**（mini-4 实测：裸 yes 会把形似神不似的专有名词错并——"Islam"→"Iman"）；prompt 携带类型与上下文事实，且明示"类型不同是反对合并的强证据"。嵌入不可用时退化为包含关系候选 + LLM 裁决；裁决调用失败则不合并。嵌入近似合并（同步 embedder 档，cos ≥ 0.9）保留为无 LLM 预算时的低配路径：
...
**事实更新的 supersede 机制**（M11 P1-B）：新事件与**同（主体实体, 谓词）**的旧事件撞车（谓词匹配容忍表面形式漂移：同主体、掩码客体后文本 Jaccard ≥ 0.8 即为同关系，谓词写法自由漂移不影响）时，一轮一次 LLM 调用按组裁决**关系基数**——"这个关系是单值（住所/职位/首都，新值取代旧值）还是多值（爱好/语言/子女，新值并列）"。模型裁决的是语义属性而非时序（时序它无法从文本判断——mini-3/4 实测逐对裁决不可靠），时序由**插入序**提供（同轮到达的冲突事实 mentionTime 相同——mini-5 实测整组漏裁的教训）。确认单值后旧值打 `supersededBy` 链接——历史完整保留、完全可逆。三道防误判防线：**重提守卫**（客体值已存在的新事件视为旧信息重提，不进裁决）、**标记传播**（旧值的重提继承 supersede 标记，防止靠提及新近度霸榜）、**多值先验**（≥3 个不同值的组直接按多值处理，不花裁决调用）。检索与呈现侧三重保护：被取代事件在现在时模式（DENSE/LAST_K）×0.3；显式历史查询（RANGE/IN_*）全分可见；top-k 内检测到的冲突组**新值居首**（模型压倒性信任列表第一项——mini-6/7 的教训）；被取代值在注入与搜索结果中带 `[superseded — newer value exists]` 标记。
...
> **更新（2026-09-11）**：本节为 r1（M9 run-2）成绩，反映本文方法在当时迭代点的水平。
> 之后 M11–M17 迭代（实体合并、supersede、NER 辅助抽取、harrier 嵌入、多跳 prompt 等）
> 后的全量重跑 r2 为 **FC-SH 85.0（89/78/90/83）/ FC-MH 51.5（31/66/55/54）/
> LME(S\*) judge 68.33**，全部审计 PASS。两轮完整对照与召回归因见
