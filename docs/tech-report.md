...
**事实更新的 supersede 机制**（M11 P1-B）：新事件与**同（主体实体, 谓词）**的旧事件撞车（谓词匹配容忍表面形式漂移：`has_headquarters` 与 `has_headquarters_in` 经谓词嵌入相似归并为同关系）时，一轮一次 LLM 调用按组裁决**关系基数**——"这个关系是单值（住所/职位/首都，新值取代旧值）还是多值（爱好/语言/子女，新值并列）"。模型裁决的是语义属性而非时序（时序它无法从文本判断——mini-3/4 实测逐对裁决不可靠），时序由提及顺序提供。确认单值后旧值打 `supersededBy` 链接——历史完整保留、完全可逆。三道防误判防线：**重提守卫**（客体值已存在的新事件视为旧信息重提，不进裁决）、**标记传播**（旧值的重提继承 supersede 标记，防止靠提及新近度霸榜）、**多值先验**（≥3 个不同值的组直接按多值处理，不花裁决调用）。检索侧：被取代事件在现在时模式（DENSE/LAST_K）×0.3，显式历史查询（RANGE/IN_*）全分可见。
