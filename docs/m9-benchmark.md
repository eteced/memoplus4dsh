...
- **FC-SH 6k 完成：EM 83.0（100 题）**、**FC-SH 32k 完成：EM 81.0（100 题）**——均远超 Table 2 全体（最高 GPT-4o 60.0；RAG/记忆类最高 HippoRAG-v2 54.0、Mem0 18.0）。且 6k→32k 几乎无衰减（83.0→81.0），相比论文中"长度增加性能骤降"的普遍现象（o4-mini 80.0→14.0），本组合的实体-时间图 + 状态去重架构在 Selective Forgetting 轴上表现突出。注意 o4-mini 的 6k FC-SH/MH 也是高分——推理骨架有贡献，最终结论需看 64k/262k 与 MH。
